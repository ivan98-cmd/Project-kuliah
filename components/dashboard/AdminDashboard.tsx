...
import { Award, Bell, CalendarDays, CheckCircle2, ClipboardList, Settings, Users } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const summary = useMemo(
    () => ({
      ...
      filledEvents: events.filter((event) => event.sisaKuota === 0).length,
      checkedIn: participants.filter((item) => item.status_kehadiran === 'Hadir').length,
    }),
    [events, participants]
  );
  ...
  const renderContent = () => {
        ...
        <div className="space-y-6">
          <div className="grid gap-6 xl:grid-cols-4">
            ...
            <div className="rounded-[32px] border border-white/10 bg-[#1b1639]/80 p-6 shadow-card">
              ...
              <p className="mt-4 text-3xl font-semibold text-white">{summary.filledEvents}</p>
            </div>
            <div className="rounded-[32px] border border-white/10 bg-[#1b1639]/80 p-6 shadow-card">
              <p className="text-sm text-slate-400">Hadir Dikonfirmasi</p>
              <p className="mt-4 text-3xl font-semibold text-white">{summary.checkedIn}</p>
            </div>
          </div>
          <div className="grid gap-6 xl:grid-cols-[1.5fr_1fr]">
            <div className="rounded-[32px] border border-white/10 bg-[#1b1639]/80 p-6 shadow-card">
              <p className="text-sm uppercase tracking-[0.24em] text-slate-400">Ringkasan Administrator</p>
              <p className="mt-4 text-slate-300">Kelola event, peserta, dan laporan untuk menjaga kualitas platform dan pengalaman pengguna.</p>
            </div>
            <div className="rounded-[32px] border border-white/10 bg-[#1b1639]/80 p-6 shadow-card">
              <p className="text-sm uppercase tracking-[0.24em] text-slate-400">Status Sistem</p>
              <p className="mt-4 text-slate-300">Aktifkan event terbaru, pantau kehadiran, dan pastikan setiap event berjalan tanpa gangguan.</p>
            </div>
          </div>
          <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
            ...
            <div className="space-y-6">
              <DonutChartBudget />
              <div className="rounded-[32px] border border-white/10 bg-[#1b1639]/80 p-6 shadow-card">
                <p className="text-sm uppercase tracking-[0.24em] text-slate-400">Laporan ringkas</p>
                <p className="mt-4 text-slate-300">Pantau event dan peserta untuk memprioritaskan dukungan teknis dan konten paling penting.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      return (
        <div className="space-y-6">
          <div className="rounded-[32px] border border-white/10 bg-[#1b1639]/80 p-6 shadow-card">
            ...
            <p className="mt-2 text-slate-400">Tambahkan event baru dengan rundown lengkap dan lihat daftar event yang tersedia.</p>
          </div>
      ...
      return (
        <div className="space-y-6">
          <div className="rounded-[32px] border border-white/10 bg-[#1b1639]/80 p-6 shadow-card">
            ...
            <p className="mt-2 text-slate-400">Pantau status pendaftaran dan hadiri peserta pada setiap event.</p>
          </div>
          <div className="rounded-[32px] border border-white/10 bg-[#1b1639]/80 p-6 shadow-card">
      ...
      return (
        <div className="space-y-6">
          <div className="rounded-[32px] border border-white/10 bg-[#1b1639]/80 p-6 shadow-card">
            ...
            <h2 className="mt-4 text-3xl font-semibold text-white">Laporan platform</h2>
            <p className="mt-2 text-slate-400">Lihat metrik event dan peserta dalam tampilan ringkas.</p>
          </div>
          <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
            <BarChartEvents />
            <LineChartFinance />
      ...
      return (
        <div className="space-y-6">
          <div className="rounded-[32px] border border-white/10 bg-[#1b1639]/80 p-6 shadow-card">
            ...
            <p className="mt-2 text-slate-400">Kelola data akun dan preferensi keamanan platform.</p>
          </div>
          <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
            ...
            <div className="rounded-[32px] border border-white/10 bg-[#1b1639]/80 p-8 shadow-card">
              <p className="text-sm text-slate-400">Bantuan</p>
    ...
    return (
      <div className="rounded-[32px] border border-white/10 bg-[#1b1639]/80 p-6 shadow-card">
        ...
        <p className="mt-2 text-slate-400">Kelola event, laporan, peserta, dan sumber daya secara efisien.</p>
      </div>
    );
  };
          ...
          <div className="flex flex-col gap-6 xl:gap-8">
            <header className="flex flex-col gap-6 rounded-[32px] border border-white/10 bg-[#140f33]/80 px-6 py-6 shadow-card backdrop-blur-xl md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">Kelola event, peserta, rundown, laporan, dan AI Assistant dalam satu tempat.</p>
              </div>
